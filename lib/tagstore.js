...
function createDefaultTree() {
  ...
  for (const type of iterateStoredTypes()) {
    defaultTree[type] = []
  }
  return defaultTree
}
...
class TagStore extends LowStore {
  ...
  * filterTags(filterString) {
  ...
  randomItem(type) {
    return this.queryTable(type, table => table.sample())
  }
        ...
        if (getTagType(term) === "category") { //search free text (also note and label)
          for (const textField of ["label", "note"]) {
            if (textField in task) {
              if (task[textField].includes(term)) {
                yield task
                break termLoop
              }
            }

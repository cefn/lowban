  ...
  const allResolvers = {
    ...
    Mutation: {
      taskMerge: (_parent, args) => {
        tagStore.mergeItem("task", args.input)
      },
      tagMerge: (_parent, args) => {
        tagStore.mergeItem("tag", args.input)
      },
      taskSnooze: (_parent, args) => {
        const { id, until } = args
        tagStore.addTaskActionById(id, "snooze", { until })
      },
      taskFulfil: (_parent, args) => {
        tagStore.addTaskActionById(args.id, "fulfil")
      }
    },
    Task: {

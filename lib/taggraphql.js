...
function resolverFactory(tagStore) {
  const queryResolvers = {
    ...
    task: (parent, args) => tagStore.getById("task", args.id),
    context: (parent, args) => tagStore.getById("context", args.id),
    priority: (parent, args) => tagStore.getById("priority", args.id),
    status: (parent, args) => tagStore.getById("status", args.id),
    category: (parent, args) => tagStore.getById("category", args.id),
  }
  ...
  const mutationResolvers = {
    taskSave: (parent, args) => {
      return tagStore.saveItem("task", args.input)
    }
  }
